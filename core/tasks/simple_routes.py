...
async def retrieve_tasks_details(task_id: int = Path(..., gt=0), db: Session = Depends(get_db)):
...
async def remove_task(task_id: int = Path(..., gt=0), db: Session = Depends(get_db)):
    task_obj = db.query(TaskModel).filter_by(id=task_id).first()
    if task_obj is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found")
    ...
    db.delete(task_obj)
    db.commit()
    return JSONResponse(content='task remove successfully', status_code=status.HTTP_200_OK)
